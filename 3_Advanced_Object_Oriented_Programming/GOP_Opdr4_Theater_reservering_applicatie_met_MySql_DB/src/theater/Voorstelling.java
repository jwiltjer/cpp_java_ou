...
public class Voorstelling {
	...
	/**
	 * Reserveert deze plaats. Voorwaarde: de plaats is nog vrij
	 * 
	...
	/**
	 * Verander reserveringsstatus van stoel (VRIJ <=> GERESERVEERD). Voorwaarde:
	 * rijnummer en stoelnummer vallen binnen de zaal
	 * 
	 * @param rijnummer   rijnummer van de plaats
	 * @param stoelnummer stoelnummer van de plaats
	...
	public void plaatsKlant(Klant klant) throws TheaterException {
		for (int i = 1; i < plaatsen.length; i++) {
			for (int j = 1; j < plaatsen[i].length; j++) {
				if (plaatsen[i][j].getStatus() == Status.GERESERVEERD) {
					Voorstellingbeheer.nieuweBezetting(datum, i, j, klant.getKlantnummer());
	 ...
	 * @return true als de zaal een plaats heeft met dit rij- en stoelnummer; anders
	 *         false
	 */
	private boolean inZaal(int rijnummer, int stoelnummer) {
		return rijnummer >= 1 && rijnummer <= Theater.AANTALRIJEN && stoelnummer >= 1
				&& stoelnummer <= Theater.AANTALPERRIJ;
	}
}
