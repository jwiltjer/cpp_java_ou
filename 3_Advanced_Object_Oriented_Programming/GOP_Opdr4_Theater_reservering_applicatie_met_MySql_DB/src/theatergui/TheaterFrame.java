...
public class TheaterFrame extends JFrame {
	...
	private void mijnInit() {
		...
		ArrayList<GregorianCalendar> data = null;
	...
	/**
	 * Event handler voor het selecteren van een voorstelling. Er hoeft alleen iets
	 * te gebeuren met de event die de voorstelling selecteert, niet met de event
	 * die de voorstelling deselecteert.
	 */
	private void voorstellingsKeuzeItemStateChanged(ItemEvent e) {
		if (e.getStateChange() == ItemEvent.SELECTED) {
			...
			try {
				...
				foutLabel.setText("omzetten datum niet gelukt");
			}
			try {
				theater.wisselVoorstelling(datum);
				theater.getHuidigeVoorstelling();
	 ...
	 * en aan het theater wordt gevraagd om de gereserveerde plaatsen aan een klant
	 * met die gegevens toe te wijzen. De invoervelden voor naam en telefoonnummer
	 * worden leeg gemaakt.
	 * 
	 * voorwaarde: naam en telefoon veld mag niet leeg zijn, naam mag alleen letters
	 * bevatten, telefoonnummer moet voldoen aan het officiele telefoonnummer
	 * format, en de plaatsen in de zaal moeten geselecteerd zijn.
	 * 
	 * @throws TheaterException
	 */
	private void plaatsKnopAction() {
		...
		foutLabel.setText("");
		if ("".equals(naam) | "".equals(telefoon)) {
			foutLabel.setText("Vul zowel een naam als telefoon nummer in");
		} else if (!naam.matches("[a-zA-Z]+")) {
			foutLabel.setText("een naam mag alleen letters bevatten");
		} else if (!telefoon.matches(
				"(^\\+[0-9]{2}|^\\+[0-9]{2}\\(0\\)|^\\(\\+[0-9]{2}\\)\\(0\\)|^00[0-9]{2}|^0)([0-9]{9}$|[0-9\\-\\s]{10}$)")) {
			foutLabel.setText("vul een geldig telefoon nummer in");
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			voorstellingsLabel.setFont(new Font("Dialog", Font.BOLD | Font.ITALIC, 18));
	...
	public static void main(String[] args) throws TheaterException, SQLException {
