...
public class StudentAdminFrame extends JFrame {
	...
	private void mijnInit() {
		...
		admin.addOpleiding("Wiskunde", 160);
		admin.addOpleiding("Informatica", 200);
		admin.addCPP("CPP JAVA", 6);
		admin.addCPP("CPP Software Architect", 4);
		admin.addCPP("CPP Systeem Ontwikkelaar", 3);
	...
	private void ScholerButtonAction() {
		...
		if (!naam.isEmpty()) {
			...
			foutVeld1.setText("Voer een naam in");
		}
	...
	private void puntenVeldAction() {
		...
		String puntenveld = puntenVeld.getText();
		double punten = 0.0;

		if (!puntenveld.isEmpty()) {
			punten = Double.parseDouble(puntenVeld.getText());
		}
	...
	private JComboBox<String> getOpleidingComboBox() {
		...
		// }
		// Programma[] opcom = admin.getOpleidingLijst();
		// for(int i = 0; i > opcom.length; i++) {
		// String s = opcom[i].getNaam();
		// opleidingComboBox.addItem(s);
	...
	private JLabel getFoutVeld1() {
		if (foutVeld1 == null) {
			...
			foutVeld1.setBounds(210, 108, 167, 20);
		}
		return foutVeld1;
	}
